...
/**
 * Horná lišta — VIZUÁLNY EXPERIMENT (v2), vedľa pôvodnej `TwoTierNav.tsx`.
 *
 * Pôvodná sa NEPREPISUJE a ostáva nedotknutá — prepnutie späť je jeden import
 * v `NavBar.tsx`. Logika (auto-skrývanie, roletky, mobil, klik do prázdna)
 * je zhodná; menia sa len farby a povrch:
 *
 *   1. hlbší podklad s teplým svetlom v strede namiesto plochého prechodu
 *   2. vlások zo zlatého prechodu namiesto plnej 2px linky
 *   3. sklenené rozostrenie — obsah pod lištou presvitá
 *   4. zlaté podčiarknutie kategórie, ktoré vyrastie od stredu
 *   5. tenký ukazovateľ prečítanej časti stránky po spodnej hrane
 */
...
// ── Design tokens (2A) ───────────────────────────────────────────────────────
/* Dve vrstvy namiesto jedného prechodu: teplé svetlo v strede (akoby zhora
   dopadalo na kožu) a pod ním tmavý základ. Lišta tým prestane byť plochý pás. */
const NAV_GRADIENT =
  'radial-gradient(120% 180% at 50% -40%, rgba(200,161,90,.20) 0%, rgba(200,161,90,0) 58%),' +
  'linear-gradient(180deg,#33261a 0%,#241a12 46%,#181109 100%)';
...
/** Koľko % stránky je prečítaných — pre ukazovateľ na spodnej hrane lišty. */
...
export function TwoTierNavV2({ items, hidden = false, onHover }: TwoTierNavProps) {
  ...
  return (
    <nav
      ...
      className="two-tier-nav two-tier-nav-v2 sticky top-0 z-50"
      ...
      style={{
        ...
        // Plná 2px linka je hrubá a kreslí lištu ako škatuľu. Vlások
        // s prechodom (nižšie) drží zlato, ale hranu odľahčí.
        borderBottom: 'none',
        backdropFilter: 'blur(10px) saturate(1.15)',
        WebkitBackdropFilter: 'blur(10px) saturate(1.15)',
        boxShadow: '0 10px 30px -18px rgba(0,0,0,.85), inset 0 1px 0 rgba(255,236,196,.10)',
        transform: hidden ? 'translateY(-100%)' : 'translateY(0)',
        transition: 'transform 0.28s ease',
      }}
    >
      {/* Zlatý vlások po spodnej hrane — v strede sýty, ku krajom sa vytráca. */}
      <span
        aria-hidden="true"
        style={{
          position: 'absolute', left: 0, right: 0, bottom: 0, height: 1,
          background: `linear-gradient(90deg, transparent, ${NAV_BORDER_GOLD} 18%, #e9c877 50%, ${NAV_BORDER_GOLD} 82%, transparent)`,
          pointerEvents: 'none',
        }}
      />
      {/* Ukazovateľ prečítanej časti stránky. Sedí na tom istom vlásku, takže
          nepridáva ďalší pruh — len ho zvýrazní po dĺžke posunu. */}
      <span
        aria-hidden="true"
        style={{
          position: 'absolute', left: 0, bottom: 0, height: 2,
          width: `${scrollPct}%`,
          background: 'linear-gradient(90deg,#c8862f,#e9c877)',
          boxShadow: '0 0 10px rgba(233,200,119,.55)',
