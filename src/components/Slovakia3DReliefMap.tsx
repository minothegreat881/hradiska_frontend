...
const CATS: { slug: CatSlug; label: string; color: string }[] = [
  { slug: 'kniezacie-sidla',            label: 'Kniežacie sídla',                color: '#C0912E' },
  { slug: 'mocenske-centra',            label: 'Mocenské centrá',                color: '#A23A2E' },
  { slug: 'strazna-funkcia',            label: 'Strážna a hospodárska funkcia',  color: '#3E7480' },
  { slug: 'refugia',                    label: 'Refúgiá',                        color: '#5E7A39' },
  { slug: 'staroveke-sidla',            label: 'Staroveké sídla',                color: '#96652F' },
  { slug: 'svatyne-a-sakralne-objekty', label: 'Svätyne a sakrálne objekty',     color: '#6C5192' },
];
...
// --- Starý pin systém (ponechaný pre zatiaľ NEVYKRESLENÉ markery/detail) ---
const PIN_UNESCO = '#E8C56E';
const PIN_FILL = { hrad: '#8B2E2E', hradisko: '#7D5A2A', zamok: '#3A2A5A' } as const;
type PinKind = keyof typeof PIN_FILL;
function pinInner(_type: PinKind): string { return ''; }
function makePinSvg(type: PinKind, isUnesco = false, size = 36): string {
  ...
  return `
    <svg xmlns="http://www.w3.org/2000/svg" width="${size}" height="${h}" viewBox="0 0 36 48" style="overflow:visible; display:block;">
      <path d="${PIN_PATH}" fill="${PIN_FILL[type]}" stroke="${stroke}" stroke-width="1.6" stroke-linejoin="round"/>
    </svg>
  `;
}
